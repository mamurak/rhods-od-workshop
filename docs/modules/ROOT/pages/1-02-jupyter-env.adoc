...
NOTE: For simplicity, we are leveraging Minio in this case as the S3 Provider and instance has already been deployed on OpenShift next to RHODS. S3 buckets and users have been already provisioned for your convenience. For production environments we recommend scalable solutions like https://www.redhat.com/en/technologies/cloud-computing/openshift-data-foundation[OpenShift Data Foundations (ODF)] or solutions from other storage vendors.

Log into the S3 Provider Console with the credentials provided and be able to see the dashboard, that looks like this:
